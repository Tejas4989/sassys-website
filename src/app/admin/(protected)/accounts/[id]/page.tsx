...
// This page is client because it has the passcode-reveal interaction after creation
export default function EditWholesaleAccountPage({ params, searchParams }: { params: any; searchParams: any }) {
  // Resolve async params in client component via use()
  const { id } = use(params as Promise<{ id: string }>);
  const sp = use(searchParams as Promise<Record<string, string>>);

  const created = sp.created === "1";
  const initialPasscode = sp.passcode ?? "";
  ...
  return (
    <div className="max-w-xl space-y-6">
      ...
      <WholesaleAccountForm
        customerId={id}
        action={(fd) => updateWholesaleAccount(id, fd)}
      />
    </div>
  );
}
