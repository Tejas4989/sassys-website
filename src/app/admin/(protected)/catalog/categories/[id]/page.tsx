...
import { updateWholesaleCategory } from "@/lib/actions/wholesale-catalog";
...
export default async function EditWholesaleCategoryPage({ params }: { params: Promise<{ id: string }> }) {
  const { id } = await params;
  const categoryId = Number(id);
  if (!Number.isInteger(categoryId)) notFound();

  const [cat] = await db.select().from(wholesaleCategories).where(eq(wholesaleCategories.id, categoryId));
  if (!cat) notFound();

  return (
    <div className="max-w-md">
      <h1 className="font-heading text-2xl font-bold mb-6">Edit Wholesale Category</h1>
      <form action={updateWholesaleCategory.bind(null, categoryId)} className="space-y-4">
